...
func (p *clusterAuditConfigParser) parse(event backend.Event) (services.Resource, error) {
	switch event.Type {
	case backend.OpDelete:
		h, err := resourceHeader(event, types.KindClusterAuditConfig, services.V2, 0)
		if err != nil {
			return nil, trace.Wrap(err)
		}
