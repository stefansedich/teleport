...
A common approach involves restricting the open ports to only the most minimal ones required. Our [Getting Started](./getting-started.mdx) demonstrates how to close *all* ports for a protected resource while still granting access through Teleport.
...
Divided topologies involve deliberating restricting traffic, permissions, and resources to mitigate loss or access in the advent of a disaster or security event.
