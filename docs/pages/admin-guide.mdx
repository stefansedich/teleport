...
| `/var/lib/teleport` | Teleport data directory. Nodes keep their keys and certificates there. Auth servers store the audit log and the cluster keys there, but the audit log storage can be further configured via the `auth_service` section in the config file. |
...
auth_service:
  ...
  # The default keepalive interval is 300 seconds.
...
This connector type implements SAML authentication. It can be configured against any external identity manager like Okta or Auth0. This feature is only available
...
Architecture](architecture/authentication.mdx#audit-log) to learn more about how the audit logs and
session recordings are designed.
...
1. `.bytes` file or `.chunks.gz` compressed format represents the raw session bytes and is somewhat human-readable, although you are better off using [`tsh
...
can also define restrictions on user access.
